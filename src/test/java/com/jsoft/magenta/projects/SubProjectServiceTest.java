...
public class SubProjectServiceTest {
  ...
  class SubProjectCreationTests {
    ...
    public void createAssociation() {
      ...
      Mockito.when(subProjectRepository.findProjectIdById(subProject.getId()))
          .thenReturn(Optional.of(subProject.getId()));
  ...
  class SubProjectUpdateTests {
    ...
    public void updateSubProject() {
      ...
      Mockito.when(subProjectRepository.findProjectIdById(subProject.getId()))
          .thenReturn(Optional.of(project.getId()));
      Mockito.when(subProjectRepository.findById(subProject.getId()))
          .thenReturn(Optional.of(subProject));
      Mockito.when(subProjectRepository.save(subProject)).thenReturn(subProject);
    ...
    public void updateSubProjectWithExistingName() {
      ...
      Mockito.when(subProjectRepository.findProjectIdById(project.getId()))
          .thenReturn(Optional.of(1L));
      Mockito.when(
          subProjectRepository.existsByProjectIdAndName(project.getId(), subProject.getName()))
          .thenReturn(true);
      Mockito.when(subProjectRepository.findById(subProject.getId()))
          .thenReturn(Optional.of(subProject1));
      Mockito.when(subProjectRepository.save(subProject)).thenReturn(subProject);

      Assertions.assertThatThrownBy(() -> subProjectService.updateSubProject(subProject))
          .isInstanceOf(DuplicationException.class)
          .hasMessage(
              String.format("Sub-project with name %s already exists for project",
                  subProject.getName())
          );

      Mockito.verify(subProjectRepository).findProjectIdById(project.getId());
      Mockito.verify(subProjectRepository)
          .existsByProjectIdAndName(project.getId(), subProject.getName());
      Mockito.verify(subProjectRepository).findById(subProject.getId());
  ...
  class SubProjectDeleteTests {
    ...
    public void removeAssociation() {
      ...
      Mockito.verify(subProjectRepository)
          .findByIdAndUsersId(subProject.getId(), supervised.getId());
    }

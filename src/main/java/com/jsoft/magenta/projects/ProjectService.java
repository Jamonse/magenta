...
public class ProjectService {
    ...
    public Project createProject(Long accountId, Project project) { // Validate if creator is admin or has write
    ...
    public void createAssociation(Long userId, Long projectId, AccessPermission accessPermission) { // Validate if
        ...
        this.eventPublisher.publishEvent(new ProjectAssociationCreationEvent(accountId, userId, accessPermission));
        // Create association and save
        ProjectAssociation projectAssociation = new ProjectAssociation(userId, projectId, accessPermission);
    ...
    public Project updateProjectName(Long projectId, String newName) { // Validate if updater is admin or has write
    ...
    public void updateAssociation(Long userId, Long projectId, AccessPermission newPermission) {// Validate if
        ...
        this.eventPublisher.publishEvent(new ProjectAssociationUpdateEvent(projectId, userId, newPermission));
        // Update association
    ...
    public Project getProject(Long projectId) { // Find project permission of caller
        ...
        switch (accessPermission) {
            case READ: // Cannot get project information as reader
                throw new AuthorizationException("Cannot access project information with specified permission");
    ...
    public Page<Project> getAllProjects(int pageIndex, int pageSize, String sortBy, boolean asc) { // Get user id and
    ...
    public List<ProjectSearchResult> getAllProjectsResultsByNameExample(String nameExample, int resultsCount) { //
        ...
        switch (accessPermission) {
            ...
            case ADMIN: // Return all results
                return this.projectRepository.findAllResultsByNameContainingIgnoreCase(nameExample, pageRequest);
            default:
                return List.of();
        }
    }
    ...
    public Page<SubProject> getAllProjectSubProjects(
        ...
        if (accessPermission == AccessPermission.ADMIN)
            ...
            AccessPermission associationPermission = findProjectAssociationPermission(user.getId(), projectId);
            if (associationPermission == AccessPermission.READ)
                results = this.projectRepository.findAllSubProjectsByIdAndAssociationsUserId(projectId, user.getId(),
                        pageRequest);
            else
                results = this.projectRepository.findAllSubProjectsById(projectId, pageRequest);
        }
    ...
    public List<SubProjectSearchResult> getProjectSubProjectResults(Long projectId, int resultsCount) {
        PageRequest pageRequest = PageRequestBuilder.buildPageRequest(
                0, resultsCount, AppDefaults.PROJECTS_DEFAULT_SORT, false);
        User user = securityService.currentUser();
        AccessPermission accessPermission = user.getProjectPermission();
    ...
    public List<SubProjectSearchResult> getProjectSubProjectResultsByNameExample(Long projectId, String nameExample,
                                                                                 int resultsCount) {
        PageRequest pageRequest = PageRequestBuilder.buildPageRequest(
                0, resultsCount, AppDefaults.PROJECTS_DEFAULT_SORT, false);
        User user = securityService.currentUser();
        AccessPermission accessPermission = user.getProjectPermission();
        if (accessPermission == AccessPermission.ADMIN)
            return this.projectRepository.findAllSubProjectsResultsByIdAndNameContainingIgnoreCase(projectId,
                    nameExample, pageRequest);
        AccessPermission associationPermission = findProjectAssociationPermission(user.getId(), projectId);
        if (associationPermission == AccessPermission.READ)
            return this.projectRepository.findAllSubProjectsResultsByIdAndAssociationsUserIdAndNameContainingIgnoreCase(
                    projectId, user.getId(), nameExample, pageRequest);
        return this.projectRepository.findAllSubProjectsResultsByIdAndNameContainingIgnoreCase(projectId, nameExample
                , pageRequest);
    }

    public void removeAssociation(Long userId, Long projectId) {  // Validate if remover is admin or has write
        // permission with project
        validateProjectAssociationPermission(projectId);
        removeAssociationAfterValidation(userId, projectId);
    }

    public void removeAllAssociations(Long projectId) { // Validate if remover is admin or has write permission with
        // project
        validateProjectAssociationPermission(projectId);
    ...
    public void deleteProject(Long projectId) { // Validate if remover is admin or has write permission with project
        validateProjectAssociationPermission(projectId);
    ...
    public void handleAccountAssociationUpdate(AccountAssociationUpdateEvent associationUpdateEvent) {
        boolean associatedWithProjects = this.projectAssociationRepository
                .existsByUserIdAndProjectAccountIdGreaterThanEqual(associationUpdateEvent.getAssociatedUserId(), 0L);
        if (!associatedWithProjects)
            throw new RedundantAssociationException("READ level association with account without any project is " +
                    "redundant");
    }
    ...
    @EventListener
    public void handleSubProjectAssociationRemoval(SubProjectAssociationRemovalEvent associationRemovalEvent) {
    ...
    private void removeRedundantAssociation(Long userId, Long projectId) { // Check if association exists
        this.projectAssociationRepository // If it is, check for redundancy
                .findByUserIdAndProjectId(userId, projectId)
                .ifPresent(this::checkAssociationRedundancy);
    }

    private void checkAssociationRedundancy(ProjectAssociation projectAssociation) { // Get association permission level
        AccessPermission accessPermission = projectAssociation.getPermission();
        Long userId = projectAssociation.getId().getUserId();
        if (accessPermission == AccessPermission.READ) { // For READ permission, check for associations with other
            ...
            if (!associatedWithOtherSubProjects) { // If there are no other associations
                Project project = projectAssociation.getProject(); // Publish removal event for project association
    ...
    private ProjectAssociation createReadAssociation(Long userId, Long projectId) {
        ProjectAssociation projectAssociation = new ProjectAssociation(userId, projectId, AccessPermission.READ);
        Long accountId = findAccountId(projectId);
        this.eventPublisher.publishEvent(new ProjectAssociationCreationEvent(accountId, userId, AccessPermission.READ));
        return this.projectAssociationRepository.save(projectAssociation);
    }

    private void removeAssociationAfterValidation(Long userId, Long projectId) { // Find if association with
    ...
    private AccessPermission findProjectAssociation(Long projectId) {
        ...
        return this.projectAssociationRepository
                .findAccessPermissionByUserIdAndProjectId(userId, projectId) // Otherwise find if associated with
                // account
    ...
    private boolean hasValidProjectEditPermissionPermission(Long projectId) {
        ...
        return associationPermission == AccessPermission.WRITE; // User has write permission association with account
    }

...
public class AccountService {
    ...
    public Account createAccount(
        ...
        MagentaImage cover = coverImage != null ? imageService.uploadImage(name, coverImage, MagentaImageType.COVER)
                : null;
    ...
    public void updateAssociation(Long userId, Long accountId, AccessPermission accessPermission) { // Verify that
        // association does exist
        AccountAssociation accountAssociation = findAssociation(userId, accountId);
        if (accessPermission == AccessPermission.READ) // If new permission is READ, verify that there are associated
            // projects
    ...
    public MagentaImage updateAccountImage(Long accountId, MultipartFile accountImage, MagentaImageType imageType) {
        ...
        if (image == null) { // Image does not exists -> upload a new one
            MagentaImage magentaImage = this.imageService.uploadImage(accountToUpdate.getName(), accountImage,
                    imageType);
            switch (imageType) {
                case COVER:
    ...
    public Page<Project> getAccountProjects(Long accountId, int pageIndex, int pageSize, String sortBy, boolean asc) {
        ...
        if (accessPermission == AccessPermission.ADMIN)
            ...
            if (associationPermission == AccessPermission.READ)
                results = this.accountRepository.findAllProjectsByIdAndAssociationsUserId(accountId, user.getId(),
                        pageRequest);
            else
                results = this.accountRepository.findAllProjectsById(accountId, pageRequest);
        }
    ...
    public List<ProjectSearchResult> getAccountProjectResults(Long accountId, int resultsCount) {
        ...
        AccessPermission associationPermission = findAssociation(user, accountId);
        if (associationPermission == AccessPermission.READ)
            return this.accountRepository.findAllProjectsResultsByIdAndAssociationsUserId(accountId, user.getId(),
                    pageRequest);
        return this.accountRepository.findAllProjectsResultsById(accountId, pageRequest);
    }

    public List<ProjectSearchResult> getAccountProjectResultsByNameExample(Long accountId, String nameExample,
                                                                           int resultsCount) {
        PageRequest pageRequest = PageRequestBuilder.buildPageRequest(
                0, resultsCount, AppDefaults.ACCOUNTS_DEFAULT_SORT, false);
        User user = securityService.currentUser();
        ...
        return this.accountRepository.findAllProjectsResultsByIdAndNameContainingIgnoreCase(accountId, nameExample,
                pageRequest);
    }
    ...
    public void handleAssociatedEntityEvent(AccountAssociatedEntityEvent associatedEntityEvent) {
        ...
        switch (accessPermission) {
            ...
            case WRITE:
                AccessPermission associationPermission = findAssociation(user, accountId);
                if (associationPermission == AccessPermission.READ)
                    throw new AuthorizationException("User association with account is not allowing to perform such " +
                            "operation");
            case ADMIN:
                break;
            default:
    ...
    private void removeRedundantAssociation(Long userId, Long accountId) { // Check if association exists
        this.accountAssociationRepository // If it is, check for redundancy
                .findByUserIdAndAccountId(userId, accountId)
                .ifPresent(this::checkAssociationRedundancy);
    }

    private void checkAssociationRedundancy(AccountAssociation accountAssociation) { // Get association permission level

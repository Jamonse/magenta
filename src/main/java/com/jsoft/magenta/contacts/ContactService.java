...
public class ContactService {
  ...
  public Contact createContact(Long accountId,
      Contact contact) { // Verify that account exists and permission is
    // valid by publishing event
  ...
  public Page<Contact> getAllContacts(
    ...
    if (sortBy.equals(AppDefaults.CONTACT_DEFAULT_SORT_NAME)) {
      pageRequest = PageRequestBuilder
          .buildPageRequest(pageIndex, pageSize, AppDefaults.CONTACT_DEFAULT_SORT,
              asc);
    } else {
  ...
  public List<ContactSearchResult> getAllContactsByNameExample(Long accountId, String nameExample,
      int resultsCount) {
    this.eventPublisher.publishEvent(new AccountAssociatedEntityEvent(accountId));
    PageRequest pageRequest = PageRequestBuilder.buildPageRequest(0, resultsCount,
        AppDefaults.CONTACT_DEFAULT_SORT, false);
    return this.contactRepository
        .findAllByAccountIdAndNameExample(accountId, nameExample, pageRequest);
  }

  public void deleteContact(Long contactId) {
    isContactExists(contactId);
    Long accountId = findContact(contactId).getAccount().getId();
    this.eventPublisher.publishEvent(new AccountAssociatedEntityEvent(accountId));
  ...
  private void verifyContactEmailUnique(Long accountId, String email) {
    ...
    if (exists) {
      throw new DuplicationException(
          String.format("Contact with email address %s, already exists for specified account",
              email));
    }
  }
  ...
  private void verifyContactPhoneNumberUnique(Long accountId, String phoneNumber) {
    ...
    if (exists) {
      throw new DuplicationException(
          String.format("Contact with phone number %s, already exists for specified account",
              phoneNumber));
    }
  }
